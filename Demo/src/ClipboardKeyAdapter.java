...
public class ClipboardKeyAdapter extends KeyAdapter {
    ...
    public void keyReleased(KeyEvent event) {
        if (event.isControlDown()) {
            if (event.getKeyCode()==KeyEvent.VK_C) { // Copy
                ...
                System.out.println("detected cut");
                cancelEditing();
    ...
    private void pasteFromClipboard() {
        ...
        for (String s : lines) {
            System.out.println("Paste string split into: " + s);;
        }

        if (table.getRowCount() < lines.length) {
            System.out.println("Too few rows: (" + table.getRowCount() + ") adding");
            while (table.getRowCount() < lines.length) {
